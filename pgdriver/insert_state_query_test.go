...
func TestQuery_InsertState(main *testing.T) {
	...
	main.Run("RevNotEmpty", func(t *testing.T) {
		conn := openDB(t, `postgres://postgres:postgres@localhost:5432/postgres`, ``)
		...
		err := q.InsertState(context.Background(), conn, &s)
		require.EqualError(t, err, `rev is not empty`)
	})
	...
	main.Run("Several", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Greater(t, s1.Rev, int64(0))
	...
	main.Run("Conflict", func(t *testing.T) {
		...
		s1 := flowstate.State{ID: `anID`}
		err = q.InsertState(context.Background(), conn, &s1)
		require.EqualError(t, err, `ERROR: duplicate key value violates unique constraint "flowstate_latest_states_pkey" (SQLSTATE 23505)`)
		pgErr := &pgconn.PgError{}
		require.True(t, errors.As(err, &pgErr))
		require.Equal(t, `23505`, pgErr.Code)
		require.Equal(t, `flowstate_latest_states_pkey`, pgErr.ConstraintName)
	...
	main.Run("TxOK", func(t *testing.T) {
		...
		s := flowstate.State{ID: `anID`}
		err = q.InsertState(context.Background(), tx, &s)
		require.NoError(t, err)
		require.Greater(t, s.Rev, int64(0))
	...
	main.Run("TxConflict", func(t *testing.T) {
		conn := openDB(t, `postgres://postgres:postgres@localhost:5432/postgres`, ``)
		...
		s0 := flowstate.State{ID: `anID`}
		err = q.InsertState(context.Background(), tx, &s0)
		require.NoError(t, err)
		require.Greater(t, s0.Rev, int64(0))
